ruleset gossip_node {
  meta {
    ...
    name "sensor_profile"
    shares getID, getSeq, getMessageID, makeRumor, getPeers, getReadings, makeSeen, getPeerSeen, checkSched, findRumorToSend, getState, getCurVioStatus, getMyVios, getallvios, getTotal
  }
  ...
  global {
    ...
    getMyVios = function() {
      ent:myVios.defaultsTo({})
    }
    getCurVioStatus = function() {
      ent:curVioStatus.defaultsTo("aaa")
    }
    getallvios = function() {
      ent:allVios.defaultsTo("aaa")
    }
    getTotal = function() {
      value = ent:allVios.defaultsTo({}).values().reduce(function(a,b){a + b})
      value
    }
  }
  ...
  rule reset_perrseeen {
    select when sensor soft_reset
    always {
      ent:myVios := {}
      ent:allVios := {}
    }
  ...
  rule threshold_seen {
    select when node send_seen_thresh
    always {
      raise wrangler event "send_event_on_subs" attributes {
        "domain":"node",
        "type":"hear_seen_thresh",
        "Rx_role":"node",
        "attrs":{
          "vios":ent:allVios.defaultsTo({}),
          "id":ent:myID
        }
      }
    }
  ...
  rule receive_seen_thresh {
    select when node hear_seen_thresh
    pre {
      x = event:attr("vios")
    }
    always {
      ent:allVios := ent:allVios.defaultsTo({}).put(x)
    }
  }



  rule updateVioStatus {
    select when wovyn new_temperature_reading
    pre {
      tempF = event:attrs{"temperatureF"}
      identifier = random:uuid()
      valid = (tempF.as("Number") > wb:getThreshold().as("Number")) => "y" | "n"
      otherThing = (valid == ent:curVioStatus.defaultsTo("n")) => 0 | ((ent:curVioStatus.defaultsTo("n") == "n") => 1 | -1)
      /* created = ent:allVios{getID()}
      next = (created) => created.put(identifier, otherThing) | {}.put(identifier, otherThing) */
    }
    always {
      ent:curVioStatus := valid
      ent:myVios := ent:myVios.defaultsTo({}).put(identifier, otherThing)
      ent:allVios := ent:allVios.defaultsTo({}).put(identifier, otherThing)
    }
  }
...
//RUMORS
  ...
  rule gossip_heartbeat {
    ...
    fired {
      ...
      raise node event "heartbeat_2"
    }
  }


  rule other_thing_to_do {
    select when node heartbeat_2
    pre {
      a = random:integer(1)
      doRumor = (a == 0)
    }
    if doRumor then noop();
    fired {
      raise node event "send_seen_thresh"
    }
    else {
